...
ALTER TABLE users ADD COLUMN IF NOT EXISTS access_level TEXT CHECK (access_level IN ('pending', 'approved', 'blocked', 'banned')) DEFAULT 'pending';

UPDATE users SET access_level = 'approved' WHERE access_level IS NULL;